<x-app-layout>
  <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <section class="py-4 border-b border-slate-100 dark:border-slate-700">
      <a href="{{ route('precio-envio.create') }}"
        class="bg-blue-500 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded text-sm">Agregar Precio
        Envios</a>
    </section>
    ...
    <div
      ...
      <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
        <h2 class="font-semibold text-slate-800 dark:text-slate-100 text-2xl tracking-tight">Precio Envios </h2>
      </header>
      <div class="p-3">
        ...
        <div class="overflow-x-auto">
          ...
          <table id="tabladatos" class="display text-lg" style="width:100%">
            ...
            <tbody>
              ...
              @foreach ($opcionesEnvio as $item)
                <tr>
                  <td>{{ $item->name }}</td>
                  <td>{{ $item->direcction }}</td>

                  <td class="px-3 py-2">{{ $item->price }} </td>
                  <td class="px-3 py-2">{{ $item->zip_code }} </td>
                  ...
                  <td class="flex flex-row justify-end items-center gap-5">
                    ...
                    <a href="{{ route('precio-envio.edit', $item->id) }}"
...
<script>
  $('document').ready(function() {
    ...
    $(document).on("change", ".btn_swithc", function() {
      ...
      $.ajax({
        url: "{{ route('precio-envio.updateVisible') }}",
        method: 'POST',
        data: {
    ...
    $(".btn_delete").on("click", function(e) {
      ...
      Swal.fire({
        ...
        text: "Vas a eliminar una Opcion de envio",
        icon: "warning",
