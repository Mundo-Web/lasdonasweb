                    ...
                    <form method="POST" action="">
                      ...
                      <input type="checkbox" id="hs-basic-usage"
                        class="check_d btn_swithc relative w-[3.25rem] h-7 p-px bg-gray-100 border-transparent text-transparent 
                                            ...
                                            before:bg-white checked:before:bg-blue-200 before:translate-x-0 checked:before:translate-x-full before:rounded-full before:shadow 
                                            before:transform before:ring-0 before:transition before:ease-in-out before:duration-200 dark:before:bg-gray-400 dark:checked:before:bg-blue-200"
                        id='{{ 'd_' . $item->id }}' data-field='is_active_campaing' data-idService='{{ $item->id }}'
                        data-titleService='{{ $item->name }}' {{ $item->is_active_campaing == 1 ? 'checked' : '' }}>
                      <label for="{{ 'v_' . $item->id }}"></label>
                    </form>
                  </td>
                  <td>
                    @isset($item->start_date_campaing)
                      {{ $item->start_date_campaing }} al {{ $item->end_date_campaing }} </td>
                  @endisset
