<x-app-layout>
  ...
  <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    <form action="{{ route('tipo-flor.save') }}" method="POST">
      @csrf
      <div
        class="col-span-full xl:col-span-8 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
        <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 class="font-semibold text-slate-800 dark:text-slate-100 text-2xl tracking-tight">Agregar nuevo Tipo de FLor
          </h2>
        </header>
            ...
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
              <div class="md:col-span-5">
                ...
                <div class="relative mb-2  mt-2">
                  ...
                  <input hidden class="hidden" id="id" value="{{ $tipos->id }}" name="id"></input>
                  <input type="text" id="name" name="name" value="{{ $tipos->name }}"
