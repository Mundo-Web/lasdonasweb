...
function OrderItem({ producto, extract, sku, precio, cantidad, total, usePoints, imagen, points, userPoints, img = true }) {
  ...
  return (
    <div className="flex  flex-col md:flex-row gap-5 md:gap-10 justify-between items-start w-full p-5 mt-4 border hover:border-[#336234] rounded-2xl">
      <div className="flex gap-8 min-w-[240px]  w-full">
        {img && <img loading="lazy" src={imagen} alt={producto} className="object-contain shrink-0 self-start w-28 aspect-square" />}
      ...
      <div className={`flex ${img ? 'flex-row' : 'flex-col'} justify-between items-start font-bold text-right min-w-[240px] w-full md:w-[600px] text-[#112212]`}>
